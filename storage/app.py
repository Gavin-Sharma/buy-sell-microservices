...
def get_buys(timestamp):
    ...
    # TODO query for all the events that have occurred SINCE timestamp
    rows = session.query(Buy).filter(Buy.date_created >= timestamp)

    # loop through rows. for each row call .to_dict(). Then append the dict to data
    for row in rows:
...
def get_sells(timestamp):
    # TODO: copy over code from previous version of storage
    # 8:52:00 - get get_sells() every 5 seconds
    # TODO create a session
    session = DB_SESSION()
